...
% Comenzi
\newcommand{\eng}[1]{{#1}} % Pentru cuvinte în limba engleză.
...
Obiectivul acestui proiect este dezvoltarea unui editor/compilator online de
documente \LaTeX{} cu suport pentru editarea colaborativă și administrare a
istoricului de modificări. Proiectul ar trebui să aibă și alte funcționalități
care să ajute editarea colaborativă.

Am ales acest subiect deoarece tipografia și \LaTeX{} se numără printre
interesele mele și am ocazia să învăț și să lucrez cu tehnologii pe care nu le-am
mai folosit.

Proiectul pe partea de server este scris ca o aplicație web în Node.js folosind
JavaScript. Aplicația rulează un serviciu în Upstart care administrează
repornirea ei dacă se închide în mod neașteptat precum și alte funcționalități
de securitate și logare.

Partea de client folosește multe biblioteci des întâlnite și este scrisă în
JavaScript și CoffeeScript (care interoperează bine). Editorul este scris
modular (în mai multe fișiere) și se folosește Browserify pentru culegerea
dependințelor și unirea într-un singur fișier \cod{.js} care este apoi minificat
pentru a-i reduce părțile inutile (comentarii, spații în plus etc).

Toate acțiunile de construire și lansare a aplicației sunt automatizate folosind
Gulp pentru a putea integra continuu ușor (de la linia de comandă) modificările
făcute proiectului.

Proiectul folosește Vagrant pentru descrierea și provizionarea mașinii virtuale sub care să
ruleze serviciul și toate celelalte programe necesare (MongoDB, \TeX{} Live etc).
Astfel proiectul poate fi rulat și dezvoltat pe orice sistem de operare și fără
a fi necesară instalarea de pachete locale (cu excepția Vagrant și VirtualBox).
În plus, în acest fel se prezintă o interfață uniformă fiecărui posibil dezvoltator.

\capfara{Contribuții}

Contribuția mea constă în proiectarea și implementarea întregii aplicații web
folosind tehnologii recente care este lansată la \cod{multilatex.com} și al
cărei surse este publicată pe GitHub\cite{multigit}.

Funcționalitățile diferite față de alte proiecte similare sunt:

\begin{itemize}

\item folosirea unei metode proprii de memorare a istoricului
(secțiunea \ref{versionaresec}, pagina \pageref{versionaresec}) folosind un
data store propriu (secțiunea \ref{filestoresec}, pagina
\pageref{filestoresec});

\item previzualizări dinamice a documentelor prin memorarea
tuturor miniaturilor de pagini (secțiunea \ref{galeriesub}, pagina
\pageref{galeriesub});

\item notificări pe proiect incluzând comunicare text cu recunoaștere contextuală
(secțiunea \ref{notificarisec}, pagina \pageref{notificarisec});

\item și navigarea secțiunilor documentului prin recunoașterea dinamică a acestora
(secțiunea \ref{documentemarisub}, pagina \pageref{documentemarisub}).
...
\item și conține un număr foarte mare de șabloane \LaTeX{}.
...
\item folosește randarea paginilor pe server și afișează imagini \acr{JPEG} pe
client (ceea ce reduce din claritatea textului);
...
\item \textbf{Node.js}, pentru rularea aplicației web;

\item \textbf{Express.js}, pentru \eng{framework}-ul web;

\item \textbf{Jade}, pentru șabloane \acr{HTML};

\item \textbf{Stylus}, pentru generarea \acr{CSS};
...
Vagrant\cite{vagrant} este un sistem de a lucra cu și de a partaja medii de dezvoltare
...
Programul construit rulează pe \eng{backend} ca un serviciu în Upstart. Upstart\cite{upstart}
...
Express\cite{express} este un \eng{framework} de aplicații web pentru Node făcut să fie
...
Jade\cite{jade} este un motor de șablonare pentru Node cu sintaxă bazată pe indentare.
...
Stylus\cite{stylus} (2010) este un limbaj \eng{stylesheet} care compilează în CSS. Este
...
\item \textbf{Backbone} pentru paradigma \acr{MVC}\cite{mvcpaper} pe client;
...
Bower\cite{bower} este un astfel de manager de pachete pentru web. Spre diferență de
...
WebSocket\cite{websocket} este un protocol de comunicare \eng{full-duplex} în principal
...
ShareJS\cite{sharejs} este \eng{framework} pentru editarea colaborativă a fișierelor text.
...
Grunt\cite{grunt} este un sistem de automatizare a proceselor de lucru cu foarte multe
...
Pachetul Browserify\cite{browserify} permite compunerea fișierelor JavaScript într-unul singur
...
Gulp\cite{gulp} este un sistem de automatizare similar, dar cu diferența principală că
...
este singurul care nu poate fi minificat.
...
Implementarea acestei aplicații web care mută un program tradițional static în
cloud se dovedește a fi una bună în contextul creșterii în utilizare a
cloud computing.
...
\item securitate, pentru că date sunt sincronizate în timp real cu serverul și
nu este nevoie de nicio salvare explicită;

\item productivitate, pentru că se poate colabora la editarea unui document și
poate fi accesat de oriunde;

\item performanță, pentru că nu necesită instalarea unei distribuții de \LaTeX{}
și nu se consumă \acr{CPU} compilând documentele;

\item și de ușurință în experimentare deoarece sunt salvate toate versiunile și
se poate duplica un proiect pentru a încerca lucruri noi.
...
Object-Oriented Programming, Volume 1, Issue 3. 1988
...
\bibitem{stylus}
\url{learnboost.github.io/stylus}

\bibitem{bower}
\url{bower.io}

\bibitem{sharejs}
\url{github.com/share/ShareJS/tree/0.6}
